...
class Meter():
    ...
    def cut_pic(self):
        ...
        ninfo = [r_1, c_x, c_y]
        return ninfo, self.cc


    def linecontours(self, cp_info, cc):
        """
        :funtion : 提取刻度线，指针
        ...
        """
        r_1, c_x, c_y = cp_info
        print(self.pname + '_resize' + self.ptype)
        self.cc = cc
        # print(self.cc == img)
        # 高斯滤波
    ...
    def decter(self, path, outpath):
        ...
        ninfo, pic = self.cut_pic()  # 2.截取表盘
        x1, x2, y1, y2 = self.linecontours(ninfo, pic)  # 提取刻度线、指针
        OZ = [x1, x2, y1, y2]
