...
class Meter():
    ...
    def needle(self, path, img, r, cx, cy, x0, y0):
        ...
        # cv2.imshow('c1', img)
        # cv2.waitKey(1000)
        # circle = np.ones(img.shape, dtype="uint`8")
        # circle = circle * 255
    ...
    def decter(self, path,outpath,opoint):
        # 零点的位置坐标
        x0 = opoint[0]
        y0 = opoint[1]
        global pname, ptype
        ...
        # 霍夫圆的圆心位置和半径
        ninfo = self.cut_pic(path)  # 2.截取表盘
        kb, r, mask = self.linecontours(ninfo)  # 提取刻度线、指针
        # 根据刻度线的斜率和截距找到中心点的集合
        point_list = self.findpoint(kb, path)
        # 找到中心点坐标
        cx, cy = self.countpoint(point_list, path)
        # 找到指针的坐标
        da, db, dc, de = self.needle(path, mask, r, cx, cy, x0, y0)
...
if __name__=="__main__":
    ...
    p0 = [258, 623]
    ang1 = mm.decter(inputpath, outputpath, p0)
    print("读数为：", ang1)