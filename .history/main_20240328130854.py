...
def Recognition():
    ...
    for item in data:
        ...
        # 表识别
            ...
            dataa['status'] = status
            # 画图
            pic = boxdraw(pic, item['location'], status, f"结果：{str(result)}{item['unit']}，状态：{'正常' if status==0 else '异常'}")
...
def boxdraw(img, location, status, text):
    ...
    # 定义文本粗细
    # 在图像上绘制文本
    return cv2AddChineseText(img, text, org, textcolor,textSize=20)
