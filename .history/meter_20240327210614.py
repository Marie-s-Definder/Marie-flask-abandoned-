...
class Meter():
    ...
    def linecontours(self, cp_info):
        ...
        """
        r_1, c_x, c_y = cp_info    
        # img = cv2.GaussianBlur(img, (3, 3), 0) #去了一个高斯滤波
        # 转换为灰度图
        ...
        y1 = int(k * x1 + b)
        y2 = int(k * x2 + b)
        cv2.line(self.Pic, (x1, y1), (x2, y2), (0, 23, 255), 1, cv2.LINE_AA)
...
if __name__=="__main__":
    ...
    pic = cv2.imread(inputpath)
    picc = Meter(pic)

    status = picc.decter(inputpath)
    print("检测结果：", status)
