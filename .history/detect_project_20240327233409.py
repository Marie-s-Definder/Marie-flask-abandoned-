...
def yolov8m(path = "liandbu.jpg", pt = 'best.pt'):
    ...
    # Use the model
    detections = model.predict(source=path, save=False)  # train the model
    # print(detections)
    # print('yes')
    ...
    for i, preset_box in enumerate(preset_boxes):
        ...
        for a,detection_box in enumerate(detections[0].boxes):
            # print(detections[0].boxes)
            # 计算检测结果框的中心点坐标
