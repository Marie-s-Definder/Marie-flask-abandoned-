...
class Meter():
    ...
    def linecontours(self, cp_info):
        ...
        self.cc = original_image
        cv2.namedWindow('circle')
        cv2.setMouseCallback('circle', self.mouse_callback)
        cv2.imshow("circle", self.cc)
        cv2.waitKey(0)
        cv2.destroyAllWindows()

        # b1, g1, r1 = cv2.split(original_image)
        # b2, g2, r2 = cv2.split(saved_image)

        # diff_blue = cv2.absdiff(b1, b2)
        # diff_green = cv2.absdiff(g1, g2)
        # diff_red = cv2.absdiff(r1, r2)

        # print(diff_blue, diff_green, diff_red)
