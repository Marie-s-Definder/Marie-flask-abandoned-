...
def Recognition():
    ...
    for item in data:
        ...
        if item['item'] == 'buttom':
            ...
            pic = boxdraw(item['location'], status, f"结果：{str(result)}，状态：{'正常' if status==0 else '异常'}")
        ...
        # 灯识别，比较特殊
            ...
            for index, (result, box2draw) in enumerate(zip(results, draw_boxes)):
                status = 1
                if item['upperLimit'][index]==result:
                    status = 0
                # 加入列表
                ...
                pic = boxdraw(box2draw, status, f"结果：{str(result)}，状态：{'正常' if status==0 else '异常'}")
            # 直接画图
