...
class Meter():
    ...
    def linecontours(self, cp_info):
        ...
        if num_channels == 3:
            # 如果是3通道的彩色图像，拆分成蓝色、绿色、红色通道
            b, g, r = cv2.split(img)

            # 创建一个空白的全黑图像，与原始图像大小相同
            blank_image = np.zeros_like(img)

            # 合并每个通道的图像到不同的颜色通道上
            blue_channel = cv2.merge([b, blank_image, blank_image])
            green_channel = cv2.merge([blank_image, g, blank_image])
            red_channel = cv2.merge([blank_image, blank_image, r])
    ...
    def decter(self, path, outpath):
        ...
        OZ = self.linecontours(ninfo)  # 提取刻度线、指针
